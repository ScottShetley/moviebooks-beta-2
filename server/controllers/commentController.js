...
const createComment = asyncHandler (async (req, res) => {
  ...
  try {
      ...
      const populatedComment = await Comment.findById (
        createdComment._id
      ).populate ('user', 'username _id profileImageUrl displayName'); // Include necessary user fields
      if (!populatedComment) {
...
const getCommentsForConnection = asyncHandler (async (req, res) => {
  ...
  // console.log ('[getComments] isValid ObjectId check result:', mongoose.Types.ObjectId.isValid (connectionId));
  // --- END DEBUG LOGS ---
  ...
  try {
      // Fetch comments using the validated connectionId, populate user, sort by creation date
      const comments = await Comment.find ({connection: connectionId})
        .populate ('user', 'username _id profileImageUrl displayName') // Include profileImageUrl and displayName
        .sort ({createdAt: 1}); // Sort ascending for discussion flow (oldest first) - Switched to 1 (ascending)
      ...
      res.status(500); // Use 500 for database fetch errors
